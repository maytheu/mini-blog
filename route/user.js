...
module.exports = app =>{
  ...
  Author.findOne({ user: "maytheu"/*req.body.email }*/}, (err, user) => {
    if (!user)
      return res.json({ loginSuccess: false, message: "User not found" });

    user.comparePassword("12345678"/*req.body.password*/, (err, isMatch) => {
      if (!isMatch)
  ...
  Author.findOneAndUpdate({ _id: req.user._id }, { token: "" }, (err, doc) => {
    if (err) return res.json({ success: false, err });
